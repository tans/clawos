...
type BrowserConnectivityStatus = "ok" | "disabled" | "not-configured" | "gateway-error" | "probe-error";
...
export async function checkBrowserConnectivity(): Promise<Record<string, unknown>> {
  ...
  const shouldProbe = configProbe.ok && browserEnabled && browserConfigured;
  const browserProbe = shouldProbe
    ...
    : null;
  const browserReady = shouldProbe && browserProbe?.ok === true;
