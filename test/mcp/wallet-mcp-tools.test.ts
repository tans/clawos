...
describe("wallet-mcp tools", () => {
  it("supports get wallet + generate wallet workflow", async () => {
    ...
    try {
      ...
      const { runTool } = await import("../../mcp/wallet-mcp/src/index");

      const before = (await runTool({ tool: "get_local_wallet" })) as { wallet: { exists: boolean } };
      expect(before.wallet.exists).toBe(false);
      ...
      const after = (await runTool({ tool: "get_local_wallet" })) as { wallet: { exists: boolean; address: string } };
      expect(after.wallet.exists).toBe(true);
      expect(after.wallet.address).toBe(generated.wallet.address);
    } finally {
