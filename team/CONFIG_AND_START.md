...
本文档说明 `company` 服务的配置、启动、首次初始化和 Agent 接入方式。

## 1. 项目位置与技术栈
- 目录：`/Users/ke/code/clawos/company`
- 运行时：`bun`
- Web 框架：`hono`
- 存储：`sqlite`
...
```bash
cd /Users/ke/code/clawos/company
```
...
| `COMPANY_DB_PATH` | `company/data/company.db` | SQLite 数据库文件路径 |

示例：

```bash
export PORT=8787
export COMPANY_DB_PATH=/data/clawos/company.db
```
...
默认会自动创建：
- `company/data/company.db`
- `company/data/company.db-shm`
- `company/data/company.db-wal`
...
## 11. 关联文档
- Company 简介：[/Users/ke/code/clawos/company/README.md](/Users/ke/code/clawos/company/README.md)
