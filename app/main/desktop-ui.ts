...
import { handlePageRequest } from "../server/pages";
import type {
  DesktopApiRequest,
  DesktopApiResponse,
  DesktopPageResponse,
...
const DESKTOP_REACT_APP_CSS_URL = "views://clawos/app.css";
const DESKTOP_REACT_APP_JS_URL = "views://clawos/react-app.js";
...
function appendDesktopBridge(html: string): string {
  ...
  output = output
    .replace(/href="\/assets\/app\.css"/g, `href="${DESKTOP_REACT_APP_CSS_URL}"`)
    .replace(/href='\/assets\/app\.css'/g, `href='${DESKTOP_REACT_APP_CSS_URL}'`)
    .replace(/src="\/assets\/react-app\.js"/g, `src="${DESKTOP_REACT_APP_JS_URL}"`)
    .replace(/src='\/assets\/react-app\.js'/g, `src='${DESKTOP_REACT_APP_JS_URL}'`);
