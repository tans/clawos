...
- `ROUTER_PORT`：监听端口（默认 `8788`）
- `ROUTER_DB_PATH`：SQLite 路径（默认 `router/data/router.db`）
- `ROUTER_CURRENCY`：计费币种（默认 `CNY`）

### 鉴权与账户

- `ROUTER_REQUIRE_AUTH`：`1` 时强制鉴权
- `ROUTER_DEFAULT_ACCOUNT_ID`：默认账户（默认 `acct_demo`）
- `ROUTER_DEFAULT_ACCOUNT_NAME`：默认账户名（默认 `Demo Account`）
- `ROUTER_API_KEYS`：逗号分隔 API key，自动绑定到默认账户
- `ROUTER_SEED_KEYS`：显式绑定 key 到账户，格式：
...
- `ROUTER_MODEL_AUTO_DEEPSEEK`
- `ROUTER_MODEL_AUTO_OPENAI`
- `ROUTER_MODEL_AUTO_ANTHROPIC`
- `ROUTER_MODEL_FAST_DEEPSEEK`
- `ROUTER_MODEL_FAST_OPENAI`
- `ROUTER_MODEL_CODE_DEEPSEEK`
- `ROUTER_MODEL_CODE_OPENAI`
- `ROUTER_MODEL_CODE_ANTHROPIC`

### 计费参数

- `ROUTER_PRICE_INPUT_PER_1M_CENTS`（默认 `200`）
- `ROUTER_PRICE_OUTPUT_PER_1M_CENTS`（默认 `800`）
- `ROUTER_STREAM_FLAT_CENTS`（默认 `1`）
- `ROUTER_PLAN_BASIC_MONTHLY_CENTS`（默认 `3900`）
- `ROUTER_PLAN_PRO_MONTHLY_CENTS`（默认 `9900`）
